...
void setup() {
  ...
  Serial.begin(115200);
   ...
   // join I2C bus (I2Cdev library doesn't do this automatically)
    ...
    tof.setScaling(SCALING);

    tof.writeReg(VL6180X::SYSRANGE__MAX_CONVERGENCE_TIME, 30);
    tof.writeReg16Bit(VL6180X::SYSALS__INTEGRATION_PERIOD, 50);

    tof.setTimeout(500);
    tof.stopContinuous();
    delay(300);
    tof.startInterleavedContinuous(100);*/
...
//       while (Serial.available()==0) {}; // Waiting char
       ...
       if (val>47 && val<67) { //from pin 0, to pin 19
          ...
          uint16_t agv = tof.readRangeContinuousMillimeters();
          //Serial.println(agv);
          //Serial.print(agv);
